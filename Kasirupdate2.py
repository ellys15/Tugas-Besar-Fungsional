...
#Pure Function
def calculate_total_bills(quantities):
    ayambakar_total = quantities["Ayam Bakar"] * harga_menu["Ayam Bakar"]
    bebekbakar_total = quantities["Bebek Bakar"] * harga_menu["Bebek Bakar"]
    sateayam_total = quantities["Sate Ayam"] * harga_menu["Sate Ayam"]
    teh_total = quantities["Es Teh"] * harga_menu["Es Teh"]
    jus_total = quantities["Orange Juice"] * harga_menu["Orange Juice"]
    lemon_total = quantities["Lemon Tea"] * harga_menu["Lemon Tea"]
...
#def update_bills(calculate_function, *spinboxes):
    #quantities = {item: int(sb.get()) for item, sb in zip(harga_menu.keys(), spinboxes)}
    #total_bills = calculate_function(quantities)
    #bills.config(text=f"Your Total Bills: Rp {total_bills}")

def button_clicked():
    quantities = {
        "Ayam Bakar": int(ayambakar_sb.get()),
        "Bebek Bakar": int(bebekbakar_sb.get()),
        "Sate Ayam": int(sateayam_sb.get()),
        "Es Teh": int(teh_sb.get()),
        "Orange Juice": int(jus_sb.get()),
        "Lemon Tea": int(lemon_sb.get()),
    }

    total_bills = calculate_total_bills(quantities)
...
# Finish Button(Lambda)
finish = Button(
    text="Order", font=("Times New Roman", 15, "normal"), command=lambda: button_clicked()
)
